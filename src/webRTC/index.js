...
export default function WebRTCChat() {
  ...
  function makePeerConnection(isInitiator = false) {
    ...
    pc.onicecandidate = (ev) => {
      if (!ev.candidate) {
        setStatus(`${isInitiator ? "Offer" : "Answer"}: ICE gathering finished`);
      }
    };
  ...
  function setupDataChannel(ch) {
    ...
    ch.onmessage = (ev) => pushMessage("peer", ev.data);
  }
  ...
  async function createAnswer() {
    try {
      if (!remoteOffer.trim()) return alert("Paste remote offer first.");
      const parsed = JSON.parse(remoteOffer.trim());
      if (pcRef.current) pcRef.current.close();
  ...
  async function applyAnswer() {
    try {
      if (!remoteAnswer.trim()) return alert("Paste remote answer first.");
      const parsed = JSON.parse(remoteAnswer.trim());
      if (!pcRef.current) return alert("Create offer first.");
  ...
  function sendMessage() {
    if (!dcRef.current || dcRef.current.readyState !== "open") return alert("Data channel not open yet.");
    ...
    <div style={{ maxWidth: 960, margin: "20px auto", fontFamily: "Arial, sans-serif" }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16 }}>
        <div style={{ border: "1px solid #ddd", padding: 12 }}>
          ...
          <textarea value={remoteAnswer} onChange={(e) => setRemoteAnswer(e.target.value)} style={{ width: "100%", height: 120, marginTop: 8 }} />
          <button onClick={applyAnswer} style={{ marginTop: 6 }}>Apply Answer (finish)</button>
        </div>
      ...
      <div style={{ marginTop: 16, border: "1px solid #ddd", padding: 12 }}>
        ...
        <div style={{ border: "1px solid #eee", height: 240, overflowY: "auto", padding: 8 }}>
          {messages.length === 0 ? <div style={{ color: "#888" }}>No messages yet.</div> :
            messages.map((m, i) => (
              <div key={i}><b>{m.who}:</b> {m.text} <span style={{ color: "#999", fontSize: 12 }}> — {m.ts}</span></div>
            ))
          }
        </div>
        <div style={{ marginTop: 8, display: "flex", gap: 8 }}>
          <input value={input} onChange={(e) => setInput(e.target.value)} placeholder="Type message..." style={{ flex: 1, padding: 8 }} />
